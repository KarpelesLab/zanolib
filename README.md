...
> The derived value **differs from the Go implementation's** `DeriveViewSecret()`, which used a hand-rolled version of the same construction with Zano's own hash-to-point, no DLEQ proofs, and broadcast partials. Any threshold wallet created against the Go derivation has a different address under this one.
